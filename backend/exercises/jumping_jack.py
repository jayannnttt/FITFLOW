"""
Jumping Jacks exercise tracker.
"""
...
class JumpingJack(BaseExercise):
    """
    Tracks jumping jack repetitions.
    Checks:
    - Partial arm raise (failing to lift arms fully above head).
    """
    def __init__(self, name: str, config: Dict[str, Any]):
        super().__init__(name, config)
        down_th = config.get("down_threshold", 150.0)
        up_th = config.get("up_threshold", 120.0)
    ...
    def validate_form(
        ...
        # Check shoulder angle relative to hips (raise arms above head)
        # Left elbow-shoulder-hip angle
        if all(kp in keypoints for kp in ["left_elbow", "left_shoulder", "left_hip"]):
            arm_ang = angle_engine.get_joint_angle(
                keypoints,
                ["left_elbow", "left_shoulder", "left_hip"],
                frame_shape
            )
            if arm_ang is not None:
                min_arm = self.config.get("checks", {}).get("partial_arm_raise", {}).get("min_arm_angle", 130.0)
                # When jumping jack arm is fully raised, elbow-shoulder-hip angle is wide
                if arm_ang < min_arm and self.rep_counter.stage == "down":
                    warnings.append(
                        FormWarning(
                            warning="Partial arm raise",
                            severity=WarningSeverity.WARNING,
                            suggestion="Raise your arms all the way up above your head."
                        )
                    )
