...
class Lunge(BaseExercise):
    """
    Tracks lunge repetitions and validates alignment.
    Checks:
    - Knee over toe (front knee drifting too far forward).
    """
    ...
    def validate_form(
        ...
        # Check front knee over toe (horizontal distance from knee to ankle should not be excessive)
        # Assuming left side lunge
        if "left_knee" in keypoints and "left_ankle" in keypoints:
            knee = keypoints["left_knee"]
            ankle = keypoints["left_ankle"]
            # If horizontal distance from knee to ankle extends past threshold (e.g. knee x is too far forward)
            # Assuming direction is facing right/left, we can check absolute delta x
            dx = abs(knee.x - ankle.x)
            if dx > 0.08:  # Normalized screen distance margin
                warnings.append(
                    FormWarning(
                        warning="Knee over toe",
                        severity=WarningSeverity.WARNING,
                        suggestion="Keep your front knee aligned directly above your ankle."
                    )
                )
