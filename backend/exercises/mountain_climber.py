"""
Mountain Climbers exercise tracker.
"""
...
class MountainClimber(BaseExercise):
    """
    Tracks mountain climber repetitions.
    Checks:
    - Bouncing hips (vertical bouncing stability check).
    """
    ...
    def update(
        ...
        if ang is not None:
            ...
            # Store hip y coordinate for bounce tracking
            if "left_hip" in keypoints:
                self.hip_y_history.append(keypoints["left_hip"].y)
                if len(self.hip_y_history) > 30:
    ...
    def validate_form(
        ...
        # Check for hip bouncing (variation in hip y coordinate)
        if len(self.hip_y_history) >= 10:
            y_span = max(self.hip_y_history) - min(self.hip_y_history)
            max_shift = self.config.get("checks", {}).get("bouncing_hips", {}).get("max_vertical_hip_shift", 40.0) / 100.0
            if y_span > max_shift:
                warnings.append(
                    FormWarning(
                        warning="Hips bouncing",
                        severity=WarningSeverity.WARNING,
                        suggestion="Keep your hips stable and lower to the ground."
                    )
                )
