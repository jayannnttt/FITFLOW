...
class HighKnees(BaseExercise):
    """
    Tracks high knees cardio exercise repetitions.
    Checks:
    - Low knee height (failing to bring knees up to waist height).
    """
    ...
    def validate_form(
        ...
        # Check hip flexion angle (hip-knee vertical position)
        if all(kp in keypoints for kp in ["left_shoulder", "left_hip", "left_knee"]):
            hip_ang = angle_engine.get_joint_angle(
                keypoints,
                ["left_shoulder", "left_hip", "left_knee"],
                frame_shape
            )
            if hip_ang is not None:
                min_hip = self.config.get("checks", {}).get("low_knee_height", {}).get("min_hip_angle", 130.0)
                # When knee is high, hip-shoulder-knee angle contracts/decreases
                if hip_ang > min_hip and self.rep_counter.stage == "down":
                    warnings.append(
                        FormWarning(
                            warning="Low knee height",
                            severity=WarningSeverity.WARNING,
                            suggestion="Drive your knees higher, aiming for waist level."
                        )
                    )
