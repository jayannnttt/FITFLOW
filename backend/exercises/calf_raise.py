...
class CalfRaise(BaseExercise):
    """
    Tracks calf raise repetitions and extension.
    Checks:
    - Insufficient height (failing to extend ankles fully).
    """
    def __init__(self, name: str, config: Dict[str, Any]):
        super().__init__(name, config)
        down_th = config.get("down_threshold", 170.0)
        up_th = config.get("up_threshold", 150.0)
    ...
    def validate_form(
        ...
        if self.angle_history:
            ...
            min_ext = self.config.get("checks", {}).get("insufficient_height", {}).get("min_extension", 155.0)
            # Calf raise ankle extension decreases angle as foot flexes/extends
            if min_ang > min_ext and len(self.angle_history) > 15:
                warnings.append(
                    FormWarning(
                        warning="Insufficient height",
                        severity=WarningSeverity.WARNING,
                        suggestion="Push up fully onto your toes to maximize calf contraction."
                    )
                )
